 ...
 * Handles network issues gracefully in React Native
 */
export const sendWelcomeEmail = async (studentData, matricNumber, defaultPassword) => {
  const MAX_RETRIES = 2;
  const TIMEOUT_MS = 30000; // 30 seconds
  const apiUrl = 'https://ftpv.appwrite.network/api/send-welcome-email';
  
  /**
   * Single attempt to send email with timeout
   */
  const attemptSend = async () => {
    console.log('📧 Calling email API for:', studentData.email);
    console.log('📤 Sending to:', apiUrl);
    
    // Create abort controller for manual timeout (React Native compatible)
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Email API returned success=false');
      }
    ...
    try {
      ...
      if (attempt === MAX_RETRIES) {
        ...
        if (error.message.includes('Network request failed')) {
          userMessage = 'Network error - email not sent';
        } else if (error.message.includes('timed out')) {
          userMessage = 'Email request timed out - please verify manually';
        } else if (error.message.includes('HTTP 4')) {
          userMessage = 'Invalid email request - please check student data';
        } else if (error.message.includes('HTTP 5')) {
          userMessage = 'Email server error - please notify student manually';
        }
...
/**
 * Optional: Send email without blocking student creation
 * Fire and forget approach
 */
export const sendWelcomeEmailAsync = (studentData, matricNumber, defaultPassword) => {
  // Don't await - let it run in background
  sendWelcomeEmail(studentData, matricNumber, defaultPassword)
    .then(result => {
      if (result.success) {
        console.log('✅ Background email sent successfully');
      } else {
        console.warn('⚠️ Background email failed:', result.error);
      }
    })
    .catch(error => {
...
//     // ✅ CRITICAL FIX: Ensure we're passing the file correctly
//     const fileToUpload = {
...
//     console.log('📦 Prepared file object:', fileToUpload);

//     const fileId = ID.unique();
    
//     console.log('🆔 Generated file ID:', fileId);
//     console.log('🗂️ Uploading to bucket:', config.bucketId);
...
//     // ✅ Get the file URL correctly
//     const fileUrl = storage.getFileView(
//       config.bucketId,
//       response.$id
//     );

//     // ✅ CRITICAL: Extract the actual URL string
//     let finalUrl;
//     if (fileUrl && typeof fileUrl === 'object') {
//       finalUrl = fileUrl.href || fileUrl.toString();
//     } else {
//       finalUrl = String(fileUrl);
//     }

//     console.log('🔗 Final file URL:', finalUrl);

//     // ✅ CRITICAL: Verify URL is a string, not object
//     if (typeof finalUrl !== 'string') {
//       console.error('❌ ERROR: finalUrl is not a string!', typeof finalUrl, finalUrl);
//       throw new Error('Failed to get valid file URL');
//     }

//     return {
//       fileId: response.$id,
//       fileUrl: finalUrl  // ✅ This MUST be a string
//     };
//   } catch (error) {
...
export const uploadProfilePicture = async (file) => {
  try {
    ...
    // ✅ CRITICAL FIX: Properly get the file URL as a string
    const fileUrlObject = storage.getFileView(
      config.bucketId,
      response.$id
    );

    console.log('📦 File URL Object:', fileUrlObject);
    console.log('📦 Type:', typeof fileUrlObject);

    // ✅ Extract the actual URL string from the URL object
    let finalUrl = '';
    
    if (fileUrlObject) {
      if (typeof fileUrlObject === 'string') {
        finalUrl = fileUrlObject;
      } else if (typeof fileUrlObject === 'object') {
        // Try different properties that might contain the URL
        finalUrl = fileUrlObject.href || 
                   fileUrlObject.toString() || 
                   String(fileUrlObject);
      }
    }

    // ✅ Additional safety: construct URL manually if needed
    if (!finalUrl || finalUrl === '[object Object]') {
      console.warn('⚠️ Could not extract URL, constructing manually...');
      const endpoint = storage.client.config.endpoint;
      const projectId = storage.client.config.project;
      finalUrl = `${endpoint}/storage/buckets/${config.bucketId}/files/${response.$id}/view?project=${projectId}`;
    }

    console.log('🔗 Final file URL (string):', finalUrl);
    console.log('🔗 URL Type:', typeof finalUrl);
    console.log('🔗 URL Length:', finalUrl.length);

    // ✅ CRITICAL: Verify URL is a valid string
    if (typeof finalUrl !== 'string' || finalUrl === '[object Object]' || finalUrl.length < 10) {
      console.error('❌ ERROR: Invalid final URL!', {
        type: typeof finalUrl,
        value: finalUrl,
        length: finalUrl?.length
      });
      throw new Error('Failed to get valid file URL string');
    }

    return {
      fileId: response.$id,
      fileUrl: finalUrl  // ✅ Guaranteed to be a valid string
    };
  } catch (error) {
    console.error('❌ Error uploading profile picture:', error);
    console.error('Error message:', error.message);
...
export const updateStudent = async (documentId, updates, newProfilePicture = null) => {
  try {
    ...
    // Handle profile picture update
    if (newProfilePicture) {
      console.log('📸 New profile picture provided, uploading...');
      
      // Get current student data to check for existing picture
      const studentResult = await getStudentById(documentId);

      // Delete old picture if exists (optional - you may want to keep history)
      if (studentResult.success && studentResult.data.profilePictureUrl) {
        console.log('🗑️ Old profile picture exists, considering deletion...');
        // Note: You'd need to extract fileId from URL or store it separately
        // For now, we'll skip deletion and just overwrite the URL
      }

      // Upload new picture
      const pictureData = await uploadProfilePicture(newProfilePicture);
      
      if (pictureData && pictureData.fileUrl) {
        pictureUrl = pictureData.fileUrl;
    ...
    // ✅ Add profile picture URL if uploaded (NOT profilePictureId)
    if (pictureUrl) {
      updateData.profilePictureUrl = pictureUrl;
      console.log('📝 Adding profilePictureUrl to update:', pictureUrl);
    }

    // Clean up empty or undefined fields that shouldn't be updated
    ...
    console.log('📝 Final update data:', {
      ...updateData,
      profilePictureUrl: updateData.profilePictureUrl ? '✅ URL provided' : '❌ No URL'
    });
    ...
    console.log('📄 Updated profilePictureUrl:', student.profilePictureUrl || '(empty)');
...
// ✅ OPTIONAL: Helper function to extract file ID from URL (if you need deletion)
...
// ✅ OPTIONAL: Delete profile picture helper
export const deleteProfilePicture = async (fileUrl) => {
  try {
    ...
    await storage.deleteFile(config.bucketId, fileId);
    console.log('✅ Profile picture deleted:', fileId);
...
//     // Generate matric number first
//     const matricNumber = await generateMatricNumber(studentData.course);
//     console.log('🆔 Generated matric number:', matricNumber);

//     // ✅ Handle profile picture upload with better validation
//     let profilePictureUrl = '';  // Default empty string
    
//     if (profilePictureFile && profilePictureFile.uri) {
//       try {
//         console.log('📸 Uploading profile picture...');
//         console.log('📋 Picture file details:', {
//           uri: profilePictureFile.uri,
//           type: profilePictureFile.mimeType || profilePictureFile.type,
//           size: profilePictureFile.fileSize
//         });
        
//         const pictureData = await uploadProfilePicture(profilePictureFile);
        
//         if (pictureData && pictureData.fileUrl) {
//           // ✅ CRITICAL: Ensure we extract the string URL
//           profilePictureUrl = String(pictureData.fileUrl);
          
//           // ✅ SAFETY CHECK: Verify it's actually a URL string
//           if (profilePictureUrl === '[object Object]' || typeof profilePictureUrl !== 'string') {
//             console.error('❌ CRITICAL ERROR: profilePictureUrl is not a valid string!');
//             console.error('Received:', profilePictureUrl);
//             console.error('pictureData:', pictureData);
...
//         console.warn('⚠️ Continuing without profile picture');
//         profilePictureUrl = ''; // Ensure empty string on error
//       }
//     } else {
//       console.log('ℹ️ No profile picture provided');
//     }

//     // ✅ SAFETY CHECK: Final verification before database write
//     if (profilePictureUrl && (profilePictureUrl === '[object Object]' || typeof profilePictureUrl !== 'string')) {
//       console.error('❌ BLOCKING: Invalid profilePictureUrl detected before database write!');
//       console.error('Value:', profilePictureUrl);
//       console.error('Type:', typeof profilePictureUrl);
//       profilePictureUrl = ''; // Force empty string to prevent corruption
//     }

//     // Create student record with unique ID
//     const studentId = ID.unique();
//     console.log('📝 Creating student document with ID:', studentId);
...
//       course: studentData.course,
//       level: studentData.level,
//       profilePictureUrl: profilePictureUrl,  // ✅ Now guaranteed to be string
//       faceTemplate: '',
//       faceImageUrl: '',
//       faceCaptured: false,
//       faceCapturedAt: null,
...
//       ...studentDocumentData,
//       profilePictureUrl: profilePictureUrl || '(empty)'
//     });
...
//     console.log('📄 Saved profilePictureUrl:', student.profilePictureUrl || '(empty)');
...
//       console.error('❌ Auth creation failed, rolling back student creation');
...
//           student.$id
//         );
//         console.log('🔄 Student record rolled back');
//       } catch (deleteError) {
...
//     console.log(`Name: ${studentData.firstName} ${studentData.surname}`);
//     console.log(`Matric: ${matricNumber}`);
//     console.log(`Email: ${studentData.email}`);
//     console.log(`Profile Picture: ${profilePictureUrl ? '✅ Yes' : '❌ No'}`);
...
//         defaultPassword: studentData.surname.trim(),
...
//     };
//   } catch (error) {
//     console.error('❌ Fatal error creating student:', error);
//     console.error('Error stack:', error.stack);
//     return { 
//       success: false, 
...
export const createStudent = async (studentData, profilePictureFile = null) => {
  try {
      ...
      try {
        ...
        if (pictureData && pictureData.fileUrl) {
          ...
          if (!profilePictureUrl || 
              profilePictureUrl === '[object Object]' || 
              typeof profilePictureUrl !== 'string' ||
              profilePictureUrl.length < 10) {
            console.error('❌ Invalid profile picture URL:', profilePictureUrl);
            throw new Error('Invalid profile picture URL format');
          }
          
          console.log('✅ Profile picture URL validated:', profilePictureUrl);
        }
      } catch (uploadError) {
        console.error('❌ Profile picture upload failed:', uploadError);
        console.warn('⚠️ Continuing without profile picture');
    ...
    const studentDocumentData = {
      ...
      course: studentData.course,
      level: studentData.level,
      profilePictureUrl: profilePictureUrl,  // ✅ Only URL, no ID
      faceTemplate: '',
      faceImageUrl: '',
      faceCaptured: false,
      faceCapturedAt: null,
    ...
    return {
      ...
      credentials: {
        ...
        message: emailResult.success 
          ? '✅ Login credentials sent to email' 
          : '⚠️ Student created (email not sent)'
      }
    };
  } catch (error) {
    console.error('❌ Fatal error creating student:', error);
...
export const hashPassword = async (password) => {
  try {
    ...
    // Use SHA256 hashing (works reliably in React Native)
...
export const createStudentAuthCredentials = async (matricNumber, surname, studentDocId) => {
  try {
    ...
    // Validate inputs
    if (!cleanMatricNumber || !cleanSurname || !studentDocId) {
      throw new Error('Missing required parameters for auth creation');
    }

    // Hash the password (surname)
...
export const adminLogout = async () => {
  try {
    // ✅ FIXED: Use the account instance, not the Account class
    await account.deleteSession('current');
    console.log('Admin logged out successfully');
    return { success: true };
  } catch (error) {
    console.error('Logout error:', error);
    // Even if logout fails, return success to clear local state
    return { success: true };
  }
};
