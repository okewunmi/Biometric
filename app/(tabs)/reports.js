...
import {
  ...
  ActivityIndicator, Alert, TextInput, RefreshControl, Platform, Modal,
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { MaterialIcons } from '@expo/vector-icons';
import DateTimePicker from '@react-native-community/datetimepicker';
// ✅ FIX 1: Import getCourseAttendanceReport, NOT getSessionAttendanceReport.
// getSessionAttendanceReport expects a session document ID (e.g. "abc123xyz")
// getCourseAttendanceReport expects (courseCode, dateString) — which is what this screen has.
...
export default function AttendanceReports() {
  ...
  /**
   * getCourseAttendanceReport items have shape:
   *   { student, attended, signedIn, signedOut, signInTime, signOutTime, duration }
   * (no signInStatus / signOutStatus fields — determine status from booleans)
   */
  const getStatusStyle = (item) => {
    if (item.signedOut) {
      return { container: styles.statusCompleted, text: styles.statusTextCompleted, label: 'Completed' };
    } else if (item.signedIn || item.attended) {
      return { container: styles.statusSignedIn, text: styles.statusTextSignedIn, label: 'Signed In' };
    }
    return { container: styles.statusAbsent, text: styles.statusTextAbsent, label: 'Absent' };
  };

  // ─── Date picker ─────────────────────────────────────────────────────────────

  const openDatePicker = () => { setTempDate(selectedDate); setShowDatePicker(true); };
  ...
  const confirmIOSDate = () => { setSelectedDate(tempDate); setShowDatePicker(false); };
  const cancelIOSDate = () => setShowDatePicker(false);

  // ─── Report generation ────────────────────────────────────────────────────────

  const handleGenerateReport = async () => {
    if (!courseCode.trim()) { Alert.alert('Error', 'Please enter a course code'); return; }

    setIsLoading(true);
    try {
      // ✅ FIX 2: call getCourseAttendanceReport(courseCode, "YYYY-MM-DD")
      // This mirrors exactly what the web page does.
      const result = await getCourseAttendanceReport(
        courseCode.toUpperCase(),
        toApiDateString(selectedDate)
      );
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    if (courseCode.trim() && report) await handleGenerateReport();
    setRefreshing(false);
  };
  ...
  const renderDatePicker = () => {
    ...
    return (
      <Modal transparent animationType="slide" visible={showDatePicker} onRequestClose={cancelIOSDate}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={cancelIOSDate}>
          <TouchableOpacity activeOpacity={1} style={styles.pickerContainer} onPress={() => {}}>
            <View style={styles.pickerToolbar}>
              <TouchableOpacity onPress={cancelIOSDate} style={styles.pickerBtn}>
                <Text style={styles.pickerBtnCancel}>Cancel</Text>
              </TouchableOpacity>
            ...
            <DateTimePicker
              ...
              onChange={onDateChange} maximumDate={new Date()} style={styles.iosDatePicker}
            />
          </TouchableOpacity>
        </TouchableOpacity>
      </Modal>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            <MaterialIcons name="assessment" size={40} color="#6366F1" />
            <Text style={styles.headerTitle}>Attendance Reports</Text>
            <Text style={styles.headerSubtitle}>View and analyze attendance data</Text>
          </View>
        </View>
        ...
        {report ? (
          <View style={styles.reportContainer}>
            ...
            {/* Stats Grid
                getCourseAttendanceReport returns:
                  stats.totalStudents, stats.present, stats.signedOut, stats.attendanceRate  */}
            <View style={styles.statsGrid}>
              <View style={[styles.statCard, styles.statCardIndigo]}>
                <Text style={styles.statLabel}>Total Students</Text>
                <Text style={styles.statValue}>{report.stats.totalStudents}</Text>
              ...
              <View style={[styles.statCard, styles.statCardGreen]}>
                <Text style={styles.statLabel}>Present</Text>
                {/* ✅ FIX 3: stats.present (not stats.signedIn) */}
                <Text style={styles.statValue}>{report.stats.present}</Text>
              </View>
              <View style={[styles.statCard, styles.statCardBlue]}>
                <Text style={styles.statLabel}>Signed Out</Text>
                <Text style={styles.statValue}>{report.stats.signedOut}</Text>
              </View>
              <View style={[styles.statCard, styles.statCardPurple]}>
                <Text style={styles.statLabel}>Attendance Rate</Text>
            ...
            <View style={styles.detailsCard}>
              ...
              {/* ✅ FIX 4: iterate report.report (not report.data)
                  Each item: { student, attended, signedIn, signedOut, signInTime, signOutTime, duration } */}
              <ScrollView style={styles.attendanceList} nestedScrollEnabled>
                {report.report.map((item, index) => {
                  const status = getStatusStyle(item);
                  const student = item.student;
                  const studentName = student
                    ? `${student.firstName ?? ''} ${student.surname ?? ''}`.trim() || 'Student'
                    : 'Student';

                  return (
                    <View key={student?.$id ?? index.toString()} style={styles.attendanceItem}>
                      <View style={styles.attendanceItemHeader}>
                        <View style={styles.attendanceItemInfo}>
                          <Text style={styles.attendanceMatric}>
                      ...
                      <View style={styles.attendanceItemDetails}>
                        ...
                        <View style={styles.attendanceDetailRow}>
                          <MaterialIcons name="schedule" size={16} color="#6B7280" />
                          <Text style={styles.attendanceDetailLabel}>Duration:</Text>
                          {/* ✅ FIX 5: item.duration (not item.totalDuration) */}
                          <Text style={styles.attendanceDetailValue}>
...
const styles = StyleSheet.create({
  ...
  backButtonText: { marginLeft: 8, fontSize: 14, color: '#6B7280' },
  ...
  generateCard: {
    backgroundColor: 'white', margin: 16, padding: 20, borderRadius: 16,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3,
  },
  generateTitle: { fontSize: 20, fontWeight: 'bold', color: '#1F2937', marginBottom: 20 },
  inputContainer: { marginBottom: 16 },
  inputLabel: { fontSize: 14, fontWeight: '500', color: '#374151', marginBottom: 8 },
  textInput: {
    borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8,
    paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, color: '#1F2937', backgroundColor: 'white',
  },
  dateInput: {
  ...
  pickerContainer: { backgroundColor: 'white', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: 24 },
  pickerToolbar: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#E5E7EB',
  },
  pickerToolbarTitle: { fontSize: 16, fontWeight: '600', color: '#1F2937' },
  pickerBtn: { paddingHorizontal: 4, paddingVertical: 4 },
  pickerBtnCancel: { fontSize: 16, color: '#6B7280' },
  pickerBtnConfirm: { fontSize: 16, color: '#6366F1', fontWeight: '600' },
  iosDatePicker: { height: 216 },
  reportContainer: { padding: 16 },
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 20 },
  statCard: {
    flex: 1, minWidth: '45%', backgroundColor: 'white', borderRadius: 12, padding: 20,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3,
  },
  statCardIndigo: { borderLeftWidth: 4, borderLeftColor: '#6366F1' },
  statCardGreen:  { borderLeftWidth: 4, borderLeftColor: '#10B981' },
  ...
  detailsHeader: { padding: 20, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  detailsTitle: { fontSize: 20, fontWeight: 'bold', color: '#1F2937' },
  detailsSubtitle: { fontSize: 13, color: '#6B7280', marginTop: 4 },
  attendanceList: { maxHeight: 600, padding: 16 },
  attendanceItem: {
  ...
  attendanceItemInfo: { flex: 1 },
  attendanceMatric: { fontSize: 16, fontWeight: 'bold', color: '#1F2937' },
  attendanceName: { fontSize: 14, color: '#6B7280', marginTop: 2 },
  attendanceItemDetails: { gap: 8 },
  attendanceDetailRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  attendanceDetailLabel: { fontSize: 13, color: '#6B7280', fontWeight: '500' },
  attendanceDetailValue: { fontSize: 13, color: '#374151' },
  statusCompleted: { backgroundColor: '#D1FAE5', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  statusTextCompleted: { color: '#065F46', fontSize: 11, fontWeight: '600' },
  statusSignedIn: { backgroundColor: '#FEF3C7', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  statusTextSignedIn: { color: '#92400E', fontSize: 11, fontWeight: '600' },
  statusAbsent: { backgroundColor: '#FEE2E2', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  statusTextAbsent: { color: '#991B1B', fontSize: 11, fontWeight: '600' },
  emptyState: {
    backgroundColor: 'white', margin: 16, padding: 48, borderRadius: 16, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3,
  },
  emptyStateText: { marginTop: 16, fontSize: 16, fontWeight: '500', color: '#6B7280', textAlign: 'center' },
});